        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="page-title-box">
                    <h4 class="page-title">Buku Ajar</h4>
                </div>
            </div>
        </div>
        ...
        <div class="row">
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            {{ Form::open(['method' => 'post', 'route' => 'buku-ajar.store', 'class' => 'form-horizontal', 'files'=> true]) }}
                                @include('admin.buku_ajar.form')
                            {{ Form::close() }}
                        </div>
                    </div><!-- /.modal-content -->
                </div><!-- /.modal-dialog -->
            ...
            <div class="col-12">
                <div class="card-box table-responsive">
                    <h4 class="m-t-0 header-title"><b>Buku Ajar</b></h4>
                    <table id="datatable" class="table table-bordered">
                        ...
                        @foreach($datas as $data)
                            <tr>
                                ...
                                <td>
                                    Penerbit : <strong>{{$data->penerbit}}</strong><br>
                                    ISBN : {{$data->isbn}}<br>
                                    Jml. Halaman : {{$data->jumlah_halaman}}
                                </td>
                                <td>
                                    @if(empty($data->dokumen))
                                        <span class="badge badge-info badge-pill">tidak ada file</span>
                                    @else
                                        <a href="buku-ajar/{{$data->dokumen}}" title="download"><i class="fa fa-download fa-lg"></i> </a>
                            ...
                            <div class="modal fade view_{{$data->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" data-backdrop="static" style="display: none;">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            {{Form::model($data,['route'=> ['lppm-buku-ajar.show', $data->id], 'method'=>'PUT', 'files'=> true])}}
                                            @include('lppm.buku_ajar.form')
                                            {{ Form::close() }}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="modal-dialog modal-sm">
                                    <div class="modal-content">
                                        <div class="modal-body">
                                            ...
                                            <form action="{{action('BukuAjarController@destroy', $data['id'])}}" method="post">
                                                {{csrf_field()}}
                            ...
                            <div class="modal fade update_{{$data->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" data-backdrop="static" style="display: none;">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            {{Form::model($data,['route'=> ['buku-ajar.update', $data->id], 'method'=>'PUT', 'files'=> true])}}
                                                @include('admin.buku_ajar.form')
                                            {{ Form::close() }}
                                        </div>
                                    </div>
