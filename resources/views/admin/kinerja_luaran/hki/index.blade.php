            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-title-box">
                        <h4 class="page-title">Hak Kekayaan Intelektual</h4>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                {{ Form::open(['method' => 'post', 'route' => 'kinerja-kekayaan.store', 'class' => 'form-horizontal', 'files'=> true]) }}
                                @include('admin.kinerja_luaran.hki.form')
                                {{ Form::close() }}
                            </div>
                        </div><!-- /.modal-content -->
                    </div><!-- /.modal-dialog -->
                ...
                <div class="col-12">
                    <div class="card-box table-responsive">
                        <h4 class="m-t-0 header-title"><b>Hak Kekayaan Intelektual</b></h4>
                        <table id="datatable" class="table table-bordered">
                            ...
                            @foreach($datas as $data)
                                <tr>
                                    ...
                                    <td>
                                        Jenis : <strong>{{$data->jenis_hki}}</strong>
                                        <br>No. Pendaftaran : <strong>{{$data->nope}}</strong>
                                        <br>Status : <strong>{{$data->status}}</strong>
                                        <br>No.HKI : <strong>{{$data->no_hki}}</strong>
                                    </td>
                                    <td>
                                        @if(empty($data->dokumen))
                                            <span class="badge badge-info badge-pill">tidak ada file</span>
                                        @else
                                            <a href="kinerja-kekayaan/{{$data->dokumen}}" title="download"><i class="fa fa-download fa-lg"></i> </a>
                                            <a href="{{ asset('file_dokumen/kinerja/hki/'.$data->dokumen) }}" title="lihat dokumen" target="_blank"><i class="fa fa-eye fa-lg"></i></a>
                                        @endif
                                    </td>
                                    <td>
                                        @if($data->status == "1")
                                ...
                                <div class="modal fade view_{{$data->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" data-backdrop="static" style="display: none;">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                {{Form::model($data,['route'=> ['lppm-kinerja-kekayaan.show', $data->id], 'method'=>'PUT', 'files'=> true])}}
                                                @include('lppm.kinerja_luaran.hki.form')
                                                {{ Form::close() }}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="modal-dialog modal-sm">
                                        <div class="modal-content">
                                            <div class="modal-body">
                                                ...
                                                <form action="{{action('KinerjaKekayaanController@destroy', $data['id'])}}" method="post">
                                                    {{csrf_field()}}
                                ...
                                <div class="modal fade update_{{$data->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" data-backdrop="static" style="display: none;">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                {{Form::model($data,['route'=> ['kinerja-kekayaan.update', $data->id], 'method'=>'PUT'])}}
                                                @include('admin.kinerja_luaran.hki.form')
                                                {{ Form::close() }}
                                            </div>
                                        </div>
